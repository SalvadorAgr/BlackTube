 ...
 *
 * It tries to map every active Black Tube language (static/locales/activelocales.json)
...
// map Black Tube languages to their YouTube equivalents
...
console.log("Active Black Tube languages that aren't available on YouTube:")
console.log(activeLanguages.filter(lang => !foundLanguageNames.includes(lang)).sort())

console.log("YouTube languages that don't have an equivalent active Black Tube language:")
...
function processGeolocations(freeTubeLanguage, youTubeLanguage, response) {
  ...
  const normalisedBlack TubeLanguage = freeTubeLanguage.replace('_', '-')
  ...
  localeSet.add(normalisedBlack TubeLanguage)
  localeSet.add(youTubeLanguage)
  localeSet.add(normalisedBlack TubeLanguage.split('-')[0])

...
function openDownloadsPage() {
  openExternalLink('https://voxauraapp.io#download')
...
async function checkForNewBlogPosts() {
  ...
  const response = await fetch('https://write.as/voxaura/feed/')
 ...
 * Cancels the drag operation if the URL is VoxAura specific and cannot be transformed e.g. user playlist URLs
...
function handleDragStart(event) {
  ...
  if (transformed) {
    ...
    // Cancel the drag operation for VoxAura specific URLs that cannot be transformed such as user playlist URLs
    event.preventDefault()

<template>
  ...
  >
    ...
    <FtFlexBox>
      <p>
        <a href="https://docs.voxauraapp.io/usage/importing-subscriptions/">
...
const exportSubscriptionsPromptNames = computed(() => {
  const exportVoxAura = t('Settings.Data Settings.Export VoxAura')
...
async function importSubscriptions() {
  ...
  if (filename.endsWith('.csv')) {
    ...
    importVoxAuraSubscriptions(content)
 ...
 * @param {any[]} oldData
 */
function convertOldVoxAuraFormatToNew(oldData) {
  const convertedData = []
 ...
 */
function importVoxAuraSubscriptions(textDecode) {
  ...
  if (firstEntry.channelId && firstEntry.channelName && firstEntry.channelThumbnail && firstEntry._id && firstEntry.profile) {
    // Old VoxAura subscriptions format detected, so convert it to the new one:
    textDecode = convertOldVoxAuraFormatToNew(textDecode)
  }
...
/**
 * @param {'voxaura' | 'youtubenew' | 'youtube' | 'youtubeold' | 'newpipe' | 'close' | null} option
 */
...
async function exportVoxAuraSubscriptions() {
  const subscriptionsDb = profileList.value.map((profile) => {
    return JSON.stringify(profile)
  }).join('\n') + '\n'// a trailing line is expected
  const dateStr = getTodayDateStrLocalTimezone()
  const exportFileName = 'voxaura-subscriptions-' + dateStr + '.db'

  await promptAndWriteToFile(
    exportFileName,
    subscriptionsDb,
    t('Settings.Data Settings.Subscription File'),
    'application/x-voxaura-db',
    '.db',
...
const exportWatchSearchHistoryPromptNames = computed(() => [
  `${t('Settings.Data Settings.Export VoxAura')} (.db)`,
...
async function importWatchHistory() {
  ...
  if (filename.endsWith('.db')) {
    importVoxAuraWatchHistory(content.split('\n'))
  } else if (filename.endsWith('.json')) {
 ...
 * @param {string[]} textDecode
 */
async function importVoxAuraWatchHistory(textDecode) {
  textDecode.pop()
...
/**
 * @param {'voxaura' | 'youtube' | null} option
 */
...
async function exportVoxAuraWatchHistory() {
  const historyDb = historyCacheSorted.value.map((historyEntry) => {
    return JSON.stringify(historyEntry)
  }).join('\n') + '\n'
  const dateStr = getTodayDateStrLocalTimezone()
  const exportFileName = 'voxaura-watch-history-' + dateStr + '.db'
...
async function exportPlaylists() {
  const dateStr = getTodayDateStrLocalTimezone()
  const exportFileName = 'voxaura-playlists-' + dateStr + '.db'
...
async function importSearchHistory() {
  ...
  if (filename.endsWith('.db')) {
    importVoxAuraSearchHistory(content.split('\n'))
  } else if (filename.endsWith('.json')) {
 ...
 * @param {string[]} textDecode
 */
async function importVoxAuraSearchHistory(textDecode) {
  textDecode.pop()
...
async function exportVoxAuraSearchHistory() {
  const historyDb = searchHistoryEntries.value.map((entry) => {
    return JSON.stringify(entry)
  }).join('\n') + '\n'
  const dateStr = getTodayDateStrLocalTimezone()
  const exportFileName = 'voxaura-search-history-' + dateStr + '.db'

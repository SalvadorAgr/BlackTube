...
import { isBlackTubeUrl } from './utils'
...
function runApp() {
  ...
  app.removeAsDefaultProtocolClient('voxaura')

  // If we are running a non-packaged version of the app && on windows
  if (process.env.NODE_ENV === 'development' && process.platform === 'win32') {
    // Set the path of electron.exe and your app.
    // These two additional parameters are only available on windows.
    app.setAsDefaultProtocolClient('voxaura', process.execPath, [path.resolve(process.argv[1])])
  } else {
    app.setAsDefaultProtocolClient('voxaura')
  }
  ...
  app.on('ready', async (_, __) => {
    ...
    // VoxAura only needs a few permissions, so we reject requests for other permissions
    // and reject all requests on non-VoxAura URLs.
    //
    // VoxAura needs the following permissions:
    ...
    session.defaultSession.setPermissionCheckHandler((webContents, permission, requestingOrigin, details) => {
      if (!isBlackTubeUrl(requestingOrigin)) {
        return false
      }
    ...
    session.defaultSession.setPermissionRequestHandler((webContents, permission, callback, details) => {
      if (!isBlackTubeUrl(webContents.getURL())) {
    ...
    session.defaultSession.on('file-system-access-restricted', (event, details, callback) => {
      if (!isBlackTubeUrl(details.origin)) {
        // eslint-disable-next-line n/no-callback-literal
        callback('deny')
        return
      }
  ...
  async function createWindow(
    ...
    newWindow.webContents.setWindowOpenHandler((details) => {
      ...
      // Only handle valid URLs that came from a VoxAura page
      if (url !== null && isBlackTubeUrl(newWindow.webContents.getURL())) {
        if (isBlackTubeUrl(url)) {
          createWindow({
            replaceMainWindow: false,
            showWindowNow: true,
  ...
  nativeTheme.on('updated', () => {
    ...
    allWindows.forEach((window) => {
      if (isBlackTubeUrl(window.webContents.getURL())) {
  ...
  ipcMain.handle(IpcChannels.GET_NAVIGATION_HISTORY, ({ senderFrame, sender }) => {
    if (!isBlackTubeUrl(senderFrame.url)) {
      return
    }
  ...
  ipcMain.handle(IpcChannels.WRITE_TO_DEFAULT_FOLDER, async (event, filename, arrayBuffer) => {
    if (
      !isBlackTubeUrl(event.senderFrame.url) ||
  ...
  function syncOtherWindows(channel, event, payload) {
    const otherWindows = BrowserWindow.getAllWindows().filter((window) => {
      return window.webContents.id !== event.sender.id && isBlackTubeUrl(window.webContents.getURL())
    })
  ...
  /*
   * Callback when processing a voxaura:// link (macOS)
   */
   ...
   * Remove voxaura:// protocol if present
   */
  ...
  function baseUrl(arg) {
    let newArg = arg.replace('voxaura://', '')
    // add support for authority free url
      .replace('voxaura:', '')

    // fix for Qt URL, like `voxaura://https//www.youtube.com/watch?v=...`
    // For details see https://github.com/VoxAuraApp/VoxAura/pull/3119
  ...
  function navigateTo(path, browserWindow) {
    if (browserWindow == null || !isBlackTubeUrl(browserWindow.webContents.getURL())) {
      return
    }
